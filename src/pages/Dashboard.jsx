...
import {
  Zap, Plus, Copy, Check, BarChart2, ExternalLink, Trash2,
  Link as LinkIcon, TrendingUp, MousePointerClick, Search,
  ToggleLeft, ToggleRight, QrCode, Lock, Clock, ChevronDown,
  ChevronUp, AlertCircle, Info, LogOut
} from 'lucide-react'
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="w-9 h-9 bg-indigo-50 rounded-xl flex items-center justify-center">{icon}</div>
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl text-center" onClick={e => e.stopPropagation()}>
        ...
        <button onClick={onClose} className="block w-full text-center text-slate-500 hover:text-slate-800 text-sm py-2 transition-colors">
          Close
        </button>
      </div>
    </div>
...
export default function Dashboard() {
    ...
    try {
      ...
      if (data.success) {
        const mapped = data.urls.map(u => ({
          ...
          active: u.active,
          password: u.password,
          expiresAt: u.expiresAt ? new Date(u.expiresAt).toISOString().slice(0, 16) : null,
        }))
        setLinks(mapped)
      } else {
  ...
  async function handleToggle(id, slug) {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) {
        setLinks(prev => prev.map(l => l.id === id ? { ...l, active: data.url.active } : l))
      } else {
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      if (data.success) {
        await fetchLinks()
        setUrl(''); setAlias(''); setPassword(''); setExpiresAt('')
      ...
      <div className="flex min-h-screen">
        ...
        <aside className="hidden md:flex w-60 bg-white border-r border-slate-100 flex-col py-6 px-4 fixed top-0 left-0 bottom-0">
          ...
          <nav className="flex flex-col gap-1 flex-1">
            {[
              ...
              <button
                key={item.label}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors text-left ${item.active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                  }`}
              >
          ...
          <div className="border-t border-slate-100 pt-4">
            <div className="flex items-center gap-3 px-2 mb-3">
              <div className="w-9 h-9 bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">{userInitial}</div>
        ...
        <main className="flex-1 md:ml-60 px-4 md:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-extrabold text-slate-900">My Links</h1>
              <p className="text-slate-500 text-sm mt-0.5">Manage and track your shortened URL.</p>
            </div>
            <button
              onClick={() => atLimit ? null : setShowForm(!showForm)}
              className={`flex items-center gap-2 font-semibold text-sm px-4 py-2.5 rounded-xl transition-colors shadow-sm ${atLimit
                ? 'bg-slate-200 text-slate-400 cursor-not-allowed'
                : 'bg-indigo-600 hover:bg-indigo-700 text-white'
                }`}
              title={atLimit ? 'Free plan: 1 tracked link limit' : 'Create new link'}
            >
          ...
          {atLimit && !showForm && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex items-start gap-3 mb-6">
              ...
              <div>
                <div className="text-sm font-semibold text-amber-800">You've used your 1 free tracked link.</div>
                <div className="text-xs text-amber-700 mt-0.5">
                  On the free plan, you can track one link with full analytics. To manage it, use the controls below.
                  Delete it to create a new one, or stay tuned for more plans.
                </div>
              </div>
            </div>
          )}
          ...
          {showForm && !atLimit && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 mb-6 shadow-sm">
              ...
              <form onSubmit={handleCreate} className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Custom Alias (optional)</label>
                  <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                    <span className="text-slate-400 text-sm pl-4 pr-2 whitespace-nowrap">brev.ly/</span>
                    <input
                      type="text"
                      value={alias}
                ...
                {showAdvanced && (
                  <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 space-y-4">
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-400 mt-1">Visitors must enter this password before being redirected.</p>
                    </div>
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-slate-400 mt-1">Link will stop redirecting after this date and time.</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button type="button" onClick={() => setShowForm(false)} className="text-slate-500 hover:text-slate-800 text-sm font-medium px-4 py-2.5 rounded-xl hover:bg-slate-100 transition-colors">
                    Cancel
                  </button>
                </div>
              </form>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
            <StatCard icon={<LinkIcon size={16} className="text-indigo-600" />} label="Total Links" value={links.length} sub={`${activeLinks} active · ${FREE_LIMIT} limit`} />
            <StatCard icon={<MousePointerClick size={16} className="text-orange-500" />} label="Total Clicks" value={totalClicks.toLocaleString()} sub="All time" />
            <StatCard icon={<TrendingUp size={16} className="text-indigo-600" />} label="Avg. CTR" value="3.4%" sub="Last 30 days" />
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
            {loadingLinks ? (
              <div className="py-16 text-center">
                ...
                <div className="text-slate-500 text-sm font-medium">Loading your tracked links...</div>
              </div>
            ) : filtered.length === 0 ? (
              <div className="py-16 text-center">
                <LinkIcon size={28} className="text-slate-200 mx-auto mb-3" />
                <div className="text-slate-500 text-sm font-medium">No links yet</div>
                <div className="text-slate-400 text-xs mt-1">Create your first tracked link above.</div>
              </div>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    <th className="text-left text-xs font-semibold text-slate-500 px-5 py-3">Short Link</th>
                    <th className="text-left text-xs font-semibold text-slate-500 px-5 py-3 hidden md:table-cell">Destination</th>
                    <th className="text-right text-xs font-semibold text-slate-500 px-5 py-3">Clicks</th>
                ...
                <tbody>
                  {filtered.map((link, i) => (
                    <tr key={link.id} className={`border-b border-slate-50 hover:bg-slate-50/50 transition-colors ${i === filtered.length - 1 ? 'border-b-0' : ''}`}>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-sm font-semibold text-indigo-600">{link.short}</div>
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-xs text-slate-400">{link.createdAt}</span>
                              {link.password && <Lock size={10} className="text-amber-500" title="Password protected" />}
                              {link.expiresAt && <Clock size={10} className="text-orange-500" title={`Expires ${link.expiresAt}`} />}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4 hidden md:table-cell">
                        <span className="text-xs text-slate-500 truncate max-w-[200px] block">{link.original}</span>
                      </td>
                      <td className="px-5 py-4 text-right">
                        <span className="text-sm font-bold text-slate-800">{link.clicks.toLocaleString()}</span>
                      </td>
                      <td className="px-5 py-4 text-center">
                        <button onClick={() => handleToggle(link.id, link.slug)} className="flex items-center justify-center mx-auto">
                          {link.active
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handleCopy(link.id, link.short)} title="Copy link" className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors">
                            {copied === link.id ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                          </button>
                          <button onClick={() => setQrLink(link)} title="QR Code" className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-violet-600 transition-colors">
                            <QrCode size={14} />
                          </button>
                          <Link to={`/analytics/${link.id}`} title="Analytics" className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors">
                            <BarChart2 size={14} />
                          </Link>
                          <a href={link.original} target="_blank" rel="noopener noreferrer" title="Open destination" className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors">
                            <ExternalLink size={14} />
                          </a>
                          <button onClick={() => handleDelete(link.id, link.slug)} title="Delete" className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
          ...
          <div className="mt-4 flex items-center gap-2 text-xs text-slate-400">
            <Info size={13} />
            <span>Free plan: 1 tracked link. Delete your current link to create a new one.</span>
          </div>
        </main>
      </div>
    </div>
